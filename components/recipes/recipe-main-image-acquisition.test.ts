...
describe("createRecipeImageRevisionSession", () => {
  test("registers the same operation for a conflict retry at the next revision", async () => {
    const acceptSnapshot = vi.fn();
    let retry: ((expectedRevision: number) => Promise<void>) | undefined;
    const session = createRecipeImageRevisionSession({
      getExpectedRevision: () => 11,
      acceptSnapshot,
      registerConflict: (_latestRevision, registeredRetry) => {
        retry = registeredRetry;
      },
    });
    const operation = vi
      .fn()
    ...
    expect(operation).toHaveBeenLastCalledWith(12);
