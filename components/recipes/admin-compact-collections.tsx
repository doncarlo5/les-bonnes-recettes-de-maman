...
export function CompactIngredientsEditor({ name, control, register }: { name: string; control: RecipeControl; register: RecipeRegister }) {
  ...
  return <FieldSet><CollectionHeader title="Ingrédients" label="Ajouter un ingrédient" onAdd={() => { append({ ...blankIngredient }); setEditingIndex(fields.length); }} />
    ...
    <Drawer open={editingIndex !== null} onOpenChange={(open: boolean) => !open && closeEditor()}><DrawerContent className="max-h-[92dvh] rounded-t-3xl">{editingIndex !== null ? <><DrawerHeader className="shrink-0 text-left"><DrawerTitle className="type-panel-title">Ingrédient {editingIndex + 1}</DrawerTitle><DrawerDescription className="">Renseigne seulement les détails utiles à la recette.</DrawerDescription></DrawerHeader><div className="grid min-h-0 flex-1 gap-3 overflow-y-auto overscroll-contain px-4 pb-4"><Field><FieldLabel>Nom</FieldLabel><Input className="h-11" {...register(`${name}.${editingIndex}.name`)} /></Field><div className="grid grid-cols-2 gap-3"><Field><FieldLabel>Quantité</FieldLabel><Input className="h-11" {...register(`${name}.${editingIndex}.quantity`)} /></Field><Field><FieldLabel>Unité</FieldLabel><Input className="h-11" {...register(`${name}.${editingIndex}.unit`)} /></Field></div><Field><FieldLabel>Notes</FieldLabel><Input className="h-11" {...register(`${name}.${editingIndex}.notes`)} /></Field></div><DrawerFooter className="shrink-0 pb-[max(1rem,env(safe-area-inset-bottom))]"><Button type="button" variant="destructive" className="min-h-11" onClick={() => { if (window.confirm("Supprimer cet ingrédient ?")) { remove(editingIndex); setEditingIndex(null); } }}><Trash2 /> Supprimer cet ingrédient</Button><Button type="button" className="min-h-12 rounded-xl" onClick={closeEditor}>Terminé</Button></DrawerFooter></> : null}</DrawerContent></Drawer>
  </FieldSet>;
}
...
export function CompactSectionsEditor({ name, control, register }: { name: string; control: RecipeControl; register: RecipeRegister }) {
  ...
  return <FieldSet><CollectionHeader title="Sections" label="Ajouter une section" onAdd={() => { append({ title: "", steps: [""] }); setEditingIndex(fields.length); }} />
    ...
    <Drawer open={editingIndex !== null} onOpenChange={(open: boolean) => !open && closeEditor()}><DrawerContent className="max-h-[92dvh] rounded-t-3xl">{editingIndex !== null ? <><DrawerHeader className="shrink-0 text-left"><DrawerTitle className="type-panel-title">Section {editingIndex + 1}</DrawerTitle><DrawerDescription className="">Organise les étapes dans leur ordre de préparation.</DrawerDescription></DrawerHeader><div className="grid min-h-0 flex-1 gap-4 overflow-y-auto overscroll-contain px-4 pb-4"><Field><FieldLabel>Titre de section</FieldLabel><Input className="h-11" {...register(`${name}.${editingIndex}.title`)} /></Field><CompactStepsEditor name={`${name}.${editingIndex}.steps`} control={control} register={register} /></div><DrawerFooter className="shrink-0 pb-[max(1rem,env(safe-area-inset-bottom))]"><Button type="button" variant="destructive" className="min-h-11" onClick={() => { if (window.confirm("Supprimer cette section ?")) { remove(editingIndex); setEditingIndex(null); } }}><Trash2 /> Supprimer la section</Button><Button type="button" className="min-h-12 rounded-xl" onClick={closeEditor}>Terminé</Button></DrawerFooter></> : null}</DrawerContent></Drawer>
  </FieldSet>;
}
