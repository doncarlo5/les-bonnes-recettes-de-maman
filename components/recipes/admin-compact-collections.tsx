...
export function CompactSectionsEditor({ name, control, register }: { name: string; control: RecipeControl; register: RecipeRegister }) {
  ...
  return <FieldSet><CollectionHeader title="Sections" label="Ajouter une section" onAdd={() => { append({ title: "", steps: [""] }); setEditingIndex(fields.length); }} />
    ...
    <Drawer open={editingIndex !== null} onOpenChange={(open: boolean) => !open && closeEditor()}><DrawerContent className="max-h-[92dvh] rounded-t-3xl">{editingIndex !== null ? <><DrawerHeader className="text-left"><DrawerTitle className="type-panel-title">Section {editingIndex + 1}</DrawerTitle><DrawerDescription className="">Organise les étapes dans leur ordre de préparation.</DrawerDescription></DrawerHeader><div className="grid gap-4 overflow-y-auto px-4 pb-4"><Field><FieldLabel>Titre de section</FieldLabel><Input className="h-11" autoFocus {...register(`${name}.${editingIndex}.title`)} /></Field><CompactStepsEditor name={`${name}.${editingIndex}.steps`} control={control} register={register} /></div><DrawerFooter className=""><Button type="button" variant="destructive" className="min-h-11" onClick={() => { if (window.confirm("Supprimer cette section ?")) { remove(editingIndex); setEditingIndex(null); } }}><Trash2 /> Supprimer la section</Button><Button type="button" className="min-h-12 rounded-xl" onClick={closeEditor}>Terminé</Button></DrawerFooter></> : null}</DrawerContent></Drawer>
  </FieldSet>;
}
